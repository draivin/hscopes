...
import * as api from '../src/hscopes';
...
suite('Scope Info', function () {
  ...
  test('api - getGrammar', async function () {
    ...
    assert.strictEqual(t.tokens.length, 11);
    assert.deepStrictEqual(t.tokens[0], {
      startIndex: 0,
      endIndex: 2,
      scopes: ['text.html.basic', 'meta.tag.sgml.html', 'punctuation.definition.tag.html'],
    });
    assert.deepStrictEqual(t.tokens[1], {
      startIndex: 2,
      endIndex: 9,
      scopes: ['text.html.basic', 'meta.tag.sgml.html', 'meta.tag.sgml.doctype.html'],
    });
    assert.deepStrictEqual(t.tokens[2], {
      startIndex: 9,
      endIndex: 14,
      scopes: ['text.html.basic', 'meta.tag.sgml.html', 'meta.tag.sgml.doctype.html'],
    });
    assert.deepStrictEqual(t.tokens[7], {
      startIndex: 21,
      endIndex: 22,
      scopes: [
        'text.html.basic',
  ...
  test('api - getScopeAt', async function () {
    ...
    assert.strictEqual(t1.text, 'DOCTYPE');
    assert.deepStrictEqual(t1.range, new vscode.Range(0, 2, 0, 9));
    assert.deepStrictEqual(t1.scopes, [
      'text.html.basic',
      'meta.tag.sgml.html',
